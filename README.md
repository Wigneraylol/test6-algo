...
Мат. док-во:
Размер корзины $S = max - min \over (n-1) <= avggap$, т.к. между $n$ элементов $n-1$ промежутков, а деление округляет вниз. Максимальный разрыв $M$ по определению $M >= avggap$. Таким образом, $M >= S$. Однако ∀ корзины $K$ $Kmax - Kmin < S$ (Для размера корзины $s$ мин элемент - $k$, макс элемент - $s + k - 1$ , их разница - $s - 1 < s$ ). Cледовательно ∀ корзины $K$ $M > Kmax - Kmin$ ч.т.д.
