...
Мат. док-во:
Размер корзины $ S = max - min / (n-1) $ <= avg_gap, т.к. между n элементов n-1 промежутков, а деление округляет вниз. Максимальный разрыв M по определению $ M >= avg_gap $. Таким образом, $ M >= S $. Однако для каждой корзины K $ Kmax - Kmin < S $ (Для размера корзины s мин элемент - k, макс элемент - s + k - 1 , их разница - s - 1 < s ). Cледовательно для каждого K $ M > Kmax - Kmin $ ч.т.д.
